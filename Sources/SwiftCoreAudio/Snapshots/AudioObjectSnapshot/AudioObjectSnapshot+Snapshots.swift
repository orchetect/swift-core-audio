...
extension AudioObjectSnapshot {
    static func _children(of object: some AudioObject) -> [AudioObjectSnapshot] {
        ...
        if object.id.rawValue == AudioSystem.shared.id.rawValue {
            ...
            let snapshots = deviceSnapshots + boxSnapshots + clockSnapshots + tapSnapshots + processSnapshots + pluginSnapshots + transportManagerSnapshots
            children = snapshots
        } else {
    ...
    static func _children(of object: some AudioObject) async -> [AudioObjectSnapshot] {
        ...
        if object.id.rawValue == AudioSystem.shared.id.rawValue {
            ...
            let snapshots = await deviceSnapshots + boxSnapshots + clockSnapshots + tapSnapshots + processSnapshots + pluginSnapshots + transportManagerSnapshots
            children = snapshots
        } else {
            // it's an audio object that isn't the System
            children = [] // TODO: parse children
